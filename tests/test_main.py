...
def test_kite_weather_good_conditions():
    with patch("main.fetch_max_wind_speed", new=AsyncMock(return_value=28.0)):
...
def test_kite_weather_too_calm():
    with patch("main.fetch_max_wind_speed", new=AsyncMock(return_value=5.0)):
        response = client.get("/kite-weather?lat=53.35&lon=-6.26")
...
def test_kite_weather_too_strong():
    with patch("main.fetch_max_wind_speed", new=AsyncMock(return_value=80.0)):
        response = client.get("/kite-weather?lat=53.35&lon=-6.26")
...
def test_weather_fetch_failure_returns_502():
    with patch("main.fetch_max_wind_speed", new=AsyncMock(side_effect=Exception("API down"))):
        response = client.get("/kite-weather?lat=53.35&lon=-6.26")
